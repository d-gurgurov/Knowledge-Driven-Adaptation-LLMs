...
\begin{tabular}{l|c:cc|cc|cc||c:cc|cc|cc}
\toprule
\multirow{2}{*}{\textbf{ISO}} & \multicolumn{9}{c}{\textbf{mBERT}} & \multicolumn{9}{c}{\textbf{XLM-R}} \\
\cmidrule(lr){2-10} \cmidrule(lr){11-19}
 &  \multicolumn{1}{c}{} & \multicolumn{3}{c}{\textbf{ConceptNet}} & \multicolumn{3}{c}{\textbf{Glot}} & \multicolumn{3}{c}{\textbf{Glot Fusion}} & 
 \multicolumn{1}{c}{} & \multicolumn{3}{c}{\textbf{ConceptNet}} & \multicolumn{3}{c}{\textbf{Glot}} & \multicolumn{3}{c}{\textbf{Glot Fusion}} \\
 \cmidrule(lr){3-5} \cmidrule(lr){6-8} \cmidrule(lr){9-11} \cmidrule(lr){12-14} \cmidrule(lr){15-17} \cmidrule(lr){18-20}
 & \textbf{Base} & \textbf{S\_bn} & \textbf{S\_bn\_inv} & \textbf{S\_bn} & \textbf{S\_bn\_inv} & \textbf{S\_bn} & \textbf{S\_bn\_inv} & \textbf{Base} & \textbf{S\_bn} & \textbf{S\_bn\_inv} \\
 \midrule
"""
    ...
    for _, row in merged_data.iterrows():
        ...
        mbert_glot_fusion_seq_bn = row.get('mBERT_Glot_Fusion_Seq_bn', '-')
        mbert_glot_fusion_seq_bn_inv = row.get('mBERT_Glot_Fusion_Seq_bn_inv', '-')
        ...
        xlm_r_glot_fusion_seq_bn = row.get('XLM-R_Glot_Fusion_Seq_bn', '-')
        xlm_r_glot_fusion_seq_bn_inv = row.get('XLM-R_Glot_Fusion_Seq_bn_inv', '-')
        
        latex += f"{language} & {mbert_conceptnet_baseline} & {mbert_conceptnet_seq_bn} & {mbert_conceptnet_seq_bn_inv} & "
        latex += f"{mbert_glot_seq_bn} & {mbert_glot_seq_bn_inv} & {mbert_glot_fusion_seq_bn} & {mbert_glot_fusion_seq_bn_inv} & "
        latex += f"{xlm_r_conceptnet_baseline} & {xlm_r_conceptnet_seq_bn}  & {xlm_r_conceptnet_seq_bn_inv} & "
        latex += f"{xlm_r_glot_seq_bn} & {xlm_r_glot_seq_bn_inv} & {xlm_r_glot_fusion_seq_bn} & {xlm_r_glot_fusion_seq_bn_inv} \\\\\n"

    # Close the LaTeX table
    latex += r"""\bottomrule
\end{tabular}
}
\caption{F1 scores comparison across different adapters for mBERT and XLM-R in ConceptNet, Glot, and Glot Fusion.}
...
if __name__ == "__main__":
    ...
    glot_fusion_files = {
    "mBERT_Glot_Fusion_Seq_bn": '/netscratch/dgurgurov/thesis/downstream_tasks/ner/glot_fusion_mbert_seq_bn_results.csv',
    "mBERT_Glot_Fusion_Seq_bn_inv": '/netscratch/dgurgurov/thesis/downstream_tasks/ner/glot_fusion_mbert_seq_bn_inv_results.csv',
    "XLM-R_Glot_Fusion_Seq_bn": '/netscratch/dgurgurov/thesis/downstream_tasks/ner/glot_fusion_xlm-r_seq_bn_results.csv',
    "XLM-R_Glot_Fusion_Seq_bn_inv": '/netscratch/dgurgurov/thesis/downstream_tasks/ner/glot_fusion_xlm-r_seq_bn_inv_results.csv'
    }

    # Combine all files
    model_files = {**conceptnet_files, **glot_files, **glot_fusion_files}
