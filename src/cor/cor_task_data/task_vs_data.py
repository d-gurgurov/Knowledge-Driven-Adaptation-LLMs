...
task_name = f"{task_dict[task]} - {model_dict[model]} - Glot"
...
norm = plt.Normalize(vmin=merged_pppl_df['Glot_GB'].min(), vmax=merged_pppl_df['Glot_GB'].max())
cmap = plt.cm.plasma
...
sc = plt.scatter(merged_pppl_df['Size (GB)'], merged_pppl_df['Average F1'], c=merged_pppl_df['Glot_GB'], cmap=cmap, norm=norm, alpha=0.6, label='F1 After')
plt.title(task_name)
...
pearson_corr_pppl, pearson_pval_pppl = pearsonr(merged_pppl_df['Size (GB)'] + merged_pppl_df['Glot_GB'], merged_pppl_df['Average F1'])
spearman_corr_pppl, spearman_pval_pppl = spearmanr(merged_pppl_df['Size (GB)'] + merged_pppl_df['Glot_GB'], merged_pppl_df['Average F1'])
...
plt.savefig(f'{task}_vs_data_{model}.png', dpi=300, bbox_inches='tight')
