...
perplexity_df = pd.read_csv('/netscratch/dgurgurov/thesis/downstream_tasks/ppl/xlm-r/flores/baseline/average_perplexities.csv')
...
excluded_languages = ["si", "mt", "ug", "am", "bo", "ku", "th"]
...
plt.title('Perplexity vs. CC100 Dataset Size')
...
for i in range(len(merged_ppl_df)):
    plt.text(merged_ppl_df['Size (GB)'].iloc[i] - 0.1, merged_ppl_df['perplexity'].iloc[i], 
...
plt.savefig('perplexity_vs_pseudo_perplexity_vs_wiki_size_xlm-r.png', dpi=300, bbox_inches='tight')
