...
def main():
    ...
    # Get the unique categories from the training set (or any other split with the "category" column)
    categories = dataset["train"].unique("category")

    # Create a mapping from category names to numerical labels
    category_to_id = {category: idx for idx, category in enumerate(categories)}

    # Function to map the category to its numerical label
    ...
    if args.language_adapter=="yes":
        ...
        model.active_adapters = Stack("lang_adapter", "topic_classification")
    else:
